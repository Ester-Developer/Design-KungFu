...
2. ✅ **Manifests written** (`k8s/`) — the same nine components (Redis, PostgreSQL as a `StatefulSet` with a `PersistentVolumeClaim` since it's the one stateful/durable piece, NATS, and the six Java services as `Deployment`s sharing one `ConfigMap` for service-discovery env vars) as Kubernetes objects, with `readinessProbe`/`livenessProbe` wired to each service's `/health` endpoint. `api-gateway`, `ws-gateway`, and `game-shard` are `LoadBalancer` Services (client-facing, reachable from outside the cluster); the rest are `ClusterIP` (internal-only, matching the same reachability split already made in `docker-compose.yml`). **Honesty note**: this environment has no reachable Kubernetes cluster (Docker Desktop's Kubernetes isn't enabled here, and installing k3s/minikube felt like the wrong unilateral call to make unattended) — the manifests are structurally validated (YAML parses, every `Service` selector matches its `Deployment`'s pod labels) but not yet applied against a live cluster. That's the next concrete step, not a "done."
3. **Next.** Apply the manifests against an actual cluster (`docker build -t kfchess:latest . && kubectl apply -f k8s/`) and re-run the same verification used for Docker Compose. Then: multiple Game Server Shard instances + the Game Allocator's least-loaded selection to prove horizontal sharding actually routes correctly (today there is one `game-shard` Deployment in both `docker-compose.yml` and `k8s/25-game-shard.yaml` — scaling it is copy/paste, see the comment in that file for why it's a second Deployment+Service rather than `replicas: N`), and HPA scaling on active-room-count (Section 2.4) once there's a metric source to scale on.
