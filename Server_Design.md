...
2. ✅ **Done — applied and verified against a live cluster.** `k8s/` holds the same nine components (Redis, PostgreSQL as a `StatefulSet` with a `PersistentVolumeClaim` since it's the one stateful/durable piece, NATS, and the six Java services as `Deployment`s sharing one `ConfigMap` for service-discovery env vars), with `readinessProbe`/`livenessProbe` wired to each service's `/health` endpoint. `api-gateway`, `ws-gateway`, and `game-shard` are `LoadBalancer` Services (client-facing); the rest are `ClusterIP` (internal-only), matching the reachability split already made in `docker-compose.yml`. Deployed to a local `kind`-provisioned cluster (Docker Desktop's Kubernetes) via `docker build -t kfchess:latest . && kubectl apply -f k8s/`: all 9 pods reached `Running`/`Ready` (the three PostgreSQL-dependent services crash-looped briefly until `postgres-0` became ready, then recovered on their own — expected without a wait-for-postgres init container, not a bug), and `run_load_test.bat` against the cluster's `LoadBalancer` addresses succeeded 6/6 across both room-code and Quick Match flows.
3. **Next.** Multiple Game Server Shard instances + the Game Allocator's least-loaded selection, to prove horizontal sharding actually routes correctly (today there is one `game-shard` Deployment in both `docker-compose.yml` and `k8s/25-game-shard.yaml` — scaling it is copy/paste, see the comment in that file for why it's a second Deployment+Service rather than `replicas: N`), and HPA scaling on active-room-count (Section 2.4) once there's a metric source to scale on.
